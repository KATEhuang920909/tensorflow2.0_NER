...
def renminribao_preprocessing(split_rate: float = 0.8,
    ...
    :return:None
    """
    path_train = os.path.join(path_renmin_dir, "example.train")
    path_test = os.path.join(path_renmin_dir, "example.test")
    if not ignore_exist and os.path.exists(path_train) and os.path.exists(path_test):
        print("人民日报数据预处理已经完成")
        return
    else:
        print("正在对人民日报数据进行预处理......")
    path_org = os.path.join(path_renmin_dir, "example.train")
